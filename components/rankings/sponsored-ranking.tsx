...
export function SponsoredRanking() {
  const t = useTranslations("sponsoredRanking");
  ...
  const fetchLeaderboard = async () => {
    ...
    let viewName = "";
    if (selectedTimeFrame === "daily") viewName = "daily_donation_leaderboard";
    else if (selectedTimeFrame === "weekly")
      viewName = "weekly_donation_leaderboard";
    else viewName = "monthly_donation_leaderboard";
    ...
    if (error || !data || data.length === 0) {
      ...
      const dummyUsers = getDummyDonationUsers(selectedTimeFrame);
      setUsers(dummyUsers);
  ...
  // Real-time subscription for trading_room_donations changes
  useEffect(() => {
    const supabase = createClient();

    // Subscribe to changes in trading_room_donations table
    const channel = supabase
      .channel("trading_room_donations_changes")
      .on(
        "postgres_changes",
        {
        ...
        (payload) => {
          ...
          // Refetch data when donation changes
          fetchLeaderboard();
        }
      )
      .subscribe();
  ...
  const filteredUsers = useMemo(
    () => users.filter((u) => (u.total_donation ?? 0) > 0),
    [users]
  );

  const topUsers = useMemo(() => {
    const demo = getDummyDonationUsers(selectedTimeFrame);
  ...
  const tableData = useMemo(() => {
    const base =
      filteredUsers.length > 0
        ? filteredUsers
        : getDummyDonationUsers(selectedTimeFrame);
    const demo = getDummyDonationUsers(selectedTimeFrame);
    const filled = [...base];
    const demoLen = demo.length || 1;
                        ...
                        <div className="relative overflow-hidden rounded-xl p-6 border border-border backdrop-blur-sm">
                          ...
                          <div className="relative z-10">
                            ...
                            <div className="flex items-center justify-center gap-2">
                              <Icons.coins className="w-6 h-6 text-yellow-500" />
      ...
      <div className="mt-16">
        ...
        <div className="hidden md:block bg-background/60 backdrop-blur-sm rounded-2xl border border-border/50 overflow-hidden shadow-xl">
          ...
          <div className="grid grid-cols-12 gap-4 px-6 py-5 bg-gradient-to-r from-muted/40 via-muted/30 to-muted/40 border-b border-border/50 font-semibold text-sm text-muted-foreground">
            ...
            <div className="col-span-3">{t("tableDonation")}</div>
