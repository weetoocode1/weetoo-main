...
export function MostFollowedRanking() {
  ...
  const fetchLeaderboard = async () => {
    ...
    let viewName = "";
    if (selectedTimeFrame === "daily") viewName = "daily_followers_leaderboard";
    else if (selectedTimeFrame === "weekly")
      viewName = "weekly_followers_leaderboard";
    else viewName = "monthly_followers_leaderboard";
    ...
    if (error || !data || data.length === 0) {
      ...
      const dummyUsers = getDummyFollowersUsers(selectedTimeFrame);
      setUsers(dummyUsers);
  ...
  // Real-time subscription for user_followers changes
  useEffect(() => {
    const supabase = createClient();

    // Subscribe to changes in user_followers table
    const channel = supabase
      .channel("user_followers_changes")
      .on(
        "postgres_changes",
        {
        ...
        (payload) => {
          ...
          // Refetch data when followers change
          fetchLeaderboard();
        }
      )
      .subscribe();
