...
export async function GET(req: NextRequest) {
  try {
    ...
    let requestPath = `/deepcoin/agents/users/rebate-list`;
    ...
    console.log("=== DeepCoin Rebate List API Request Debug ===");
    console.log("URL:", url);
    ...
    console.log("=== DeepCoin Rebate List API Response Debug ===");
    ...
    return NextResponse.json({ rebateList: data });
  } catch (error) {
    console.error("DeepCoin rebate list fetch error:", error);
    return NextResponse.json(
      { error: "Failed to fetch rebate list" },
      { status: 500 }
    );
  }
}
